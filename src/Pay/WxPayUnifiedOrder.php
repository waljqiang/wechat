...
class WxPayUnifiedOrder extends WxPayDataBase{
	...
	**/
	public function SetDetail($value){
		$this->values['detail'] = json_encode($value,JSON_UNESCAPED_UNICODE);
	}
	/**
	* 获取商品名称明细列表的值
	* @return 值
	**/
	public function GetDetail(){
		return json_decode($this->values['detail'],true);
	}
	/**
	* 判断商品名称明细列表是否存在
	* @return true 或 false
	...
	**/
	public function SetGoods_tag($value){
		$this->values['goods_tag'] = json_encode($value,JSON_UNESCAPED_UNICODE);
	}
	/**
	...
	**/
	public function GetGoods_tag(){
		return json_decode($this->values['goods_tag'],true);
	}
	/**
	...
	**/
	...
	public function SetScene_info($value){
		$this->values['scene_info'] = json_encode($value,JSON_UNESCAPED_UNICODE);
	}

	public function GetScene_info(){
		return json_decode($this->values['scene_info'],true);
	}
	...
	/**
	 * 电子发票入口开放标识Y，传入Y时，支付成功消息和支付详情页将出现开票入口
	 */
	...
	public function GetReceipt(){
		return json_decode($this->values['receipt'],true);
	}
	...
	public function GetCliIP(){
		$cip = gethostbyname($_SERVER['HTTP_HOST']);
		/*if($_SERVER["REMOTE_ADDR"]){
			$cip = $_SERVER["REMOTE_ADDR"];
		}elseif(getevn("REMOTE_ADDR")){
			$cip = getevn("REMOTE_ADDR");
		}else{
			$cip = "unkown";
		}*/
		return $cip;
